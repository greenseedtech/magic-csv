...
module.exports = CSV = (function() {
  function CSV(settings) {
    ...
    if ((_base4 = this.settings).default_col_name == null) {
      _base4.default_col_name = 'Unknown';
    }
  }
  ...
  CSV.prototype.parse = function(data, callback) {
    var allow_row, bad_rows, char, char_counts, col, col_delimiter, cols, cols_found, delimiter, delimiter_types, dup_cols, end, end_index, ending, ends, getNextColumnName, i, index, j, line, line_ending, line_index, max_field_count, min_field_count, min_index, name, new_col, new_row, newline_flag, quoted, row, seek, start, start_index, starts, v, val, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _o, _p, _ref, _ref1, _ref2, _ref3;
    ...
    if (!(data.length > 1)) {
      return callback(this._err('Line ending detection failed', 'PARSE'));
    }
    cols = data.shift();
    char_counts = {};
    delimiter_types = {
    ...
    if (!(cols.length > 1 || this.settings.allow_single_col === true)) {
      return callback(this._err('Delimiter detection failed', 'PARSE'));
    }
    ...
    if (this._blank_cols.length / cols.length >= .5 && this.settings.allow_single_col !== true) {
      return callback(this._err('Column name detection failed', 'PARSE'));
    }
    bad_rows = [];
    ...
    for (line_index = _j = 0, _len1 = data.length; _j < _len1; line_index = ++_j) {
      ...
      if (seek) {
        if (row.length - cols.length === 1 && row[row.length - 1] === '') {
          starts.pop();
    ...
    if (this._added_cols.length / cols.length >= .5) {
      return callback(this._err('Column shifting detected', 'PARSE'));
    }
  ...
  CSV.prototype._remove = function() {
  ...
  CSV.prototype._finalize = function() {
    var c, col, cols, dup_col, dup_cols, empty, empty_cols, generated, i, j, row, val, vals, _base, _base1, _base2, _base3, _base4, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
    _ref = this._rows;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      row = _ref[_i];
    ...
    for (_p = 0, _len7 = _ref4.length; _p < _len7; _p++) {
      ...
      if (generated) {
        this._remove(col, empty_cols, this._blank_cols, dup_cols);
      } else {
        this._stats.dropped_col_count++;
      }
      i = this._columns.indexOf(col);
      this._remove(i, this._columns);
      this._remove.apply(this, [i].concat(__slice.call(this._rows)));
    }
    ...
    return (_base4 = this._stats).added_col_count != null ? _base4.added_col_count : _base4.added_col_count = this._added_cols.length;
  };
