...
module.exports = CSV = (function() {
  ...
  CSV.prototype.parse = function(data, callback) {
    ...
    getNextColumnName = (function(_this) {
      return function() {
        ...
        i = 1;
        while (true) {
          col_name = "" + _this.settings.default_col_name + " " + (i++);
    ...
    this._stats.valid_col_count = cols.length;
    this._stats.blank_col_count = generated_col_count;
    bad_rows = [];
    ...
    this._finalize();
    this._stats.added_col_count = generated_col_count - this._stats.blank_col_count;
    return callback(null, this._stats);
  };
  ...
  CSV.prototype._finalize = function() {
    ...
    for (i = _k = 0, _len2 = _ref1.length; _k < _len2; i = ++_k) {
      ...
      if (empty) {
        indexes.push(i);
        this._stats.empty_cols.push(col);
      }
    }
    if (this.settings.drop_empty_cols === true && indexes.length > 0) {
      this._stats.dropped_col_count = indexes.length;
    ...
    if ((_base3 = this._stats).blank_col_count == null) {
      _base3.blank_col_count = 0;
    }
    return (_base4 = this._stats).added_col_count != null ? _base4.added_col_count : _base4.added_col_count = 0;
  };
