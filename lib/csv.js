...
module.exports = CSV = (function() {
  ...
  CSV.prototype.parse = function(data, callback) {
    var allow_row, bad_rows, char, char_counts, col_count, cols, count, delimiter, delimiter_types, end, end_index, ending, ending_counts, ending_types, ends, generated_col_count, getNextColumnName, i, index, j, line, line_ending, line_index, max_field_count, min_field_count, name, new_row, quoted, row, seek, start, start_index, starts, v, val, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _p, _q, _ref, _ref1, _ref2, _ref3;
    ...
    for (ending in ending_types) {
      name = ending_types[ending];
      count = (_ref = data.match(new RegExp(ending, 'g'))) != null ? _ref.length : void 0;
      ending_counts[name] = (count != null ? count : 0) - ending_counts.CRLF;
    }
    line_ending = '\n';
    if (ending_counts.CR > ending_counts.LF) {
      line_ending = '\r';
    }
    if (ending_counts.CRLF > ending_counts.LF) {
      line_ending = '\r\n';
    }
    this._stats.line_ending = ending_types[line_ending];
    ...
    this._stats.delimiter = delimiter_types[delimiter];
