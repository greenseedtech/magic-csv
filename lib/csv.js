...
module.exports = CSV = (function() {
  ...
  CSV.prototype.parse = function(data, callback) {
    ...
    if (!(data.length > 1)) {
      return callback(this._err('Line ending detection failed', 'NO_ROWS'));
    }
    ...
    if (!(cols.length > 1 || this.settings.allow_single_col === true)) {
      return callback(this._err('Delimiter detection failed', 'NO_DELIM'));
    }
    ...
    if (generated_col_count / cols.length >= .5) {
      return callback(this._err('Column names not found', 'NO_COLS'));
    }
    ...
    if (generated_col_count / cols.length >= .5) {
      return callback(this._err('Too many unknown columns', 'PARSE'));
    }
