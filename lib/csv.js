...
CSV = (function() {
  function CSV(settings) {
    ...
    if ((_base5 = this.settings).disable_seek == null) {
      _base5.disable_seek = false;
    }
    if ((_base6 = this.settings).allow_single_col == null) {
      _base6.allow_single_col = false;
    }
    if ((_base7 = this.settings).strict_field_count == null) {
      _base7.strict_field_count = false;
    }
    if ((_base8 = this.settings).default_col_name == null) {
      _base8.default_col_name = 'Unknown';
    }
  ...
  CSV.prototype.parse = function(data, callback) {
    var allow_row, bad_rows, char, col, col_delimiter, cols, cols_found, count, delimiter, delimiter_types, dup_cols, end, end_index, ending, ends, finish, first_row, getNextColumnName, i, index, j, line, line_ending, line_index, line_seek_count, max_char_count, max_field_count, min_field_count, min_index, name, new_col, new_row, newline_flag, quoted, row, seek, start, start_index, starts, v, val, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _o, _p, _ref, _ref1, _ref2;
    finish = (function(_this) {
      return function(err) {
        return typeof callback === "function" ? callback(err, _this._stats) : void 0;
      };
    })(this);
        ...
        if (row.length - cols.length === 1 && row[row.length - 1] === '' && this.settings.strict_field_count !== true) {
          ...
          if (line_seek_count++ > 200) {
            return this._try({
              ...
              return function(err) {
                if (err != null) {
                  return finish(_this._err('Field terminator not found'));
                }
                return finish();
              };
            })(this));
  ...
  CSV.prototype._try = function(settings, callback) {
